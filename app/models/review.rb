  ...
  def copy_answers_to(other_review)
    reviewings.each do |reviewing|
      ...
      if other_reviewing
        ...
        puts "#{reviewing.person.informal_full_name}: Found an answer sheet for this person, and copied #{num_copied} of #{reviewing.answers.count} answers over."
      else
        puts "#{reviewing.person.informal_full_name}: Could not find an answer sheet for this person."
      end
    end
  end
end
