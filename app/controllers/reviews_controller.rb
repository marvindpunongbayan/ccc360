class ReviewsController < AnswerSheetsController
  ...
  def new_submit_form
    ...
    redirect_to new_subject_reviews_path(:format => "js"), :protocol => "https://"
  end

  def new_submit_subject
    session[:new_review][:subject_id] = params[:subject_id]
    redirect_to new_initiator_reviews_path(:format => "js"), :protocol => "https://"
  end

  def new_submit_initiator
    session[:new_review] ||= {}
    session[:new_review][:initiator_id] = params[:initiator_id]
    redirect_to new_details_reviews_path(:format => "js"), :protocol => "https://"
  end

  def show
    unless request.xhr?
      redirect_to collate_review_path(params[:id]), :protocol => "https://"
    else
